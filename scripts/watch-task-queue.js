...
function isDssRoot(candidate) {
    try {
        return fs.statSync(candidate).isDirectory() &&
            fs.statSync(path.join(candidate, ".codex_profile")).isDirectory() &&
...
function loadConfig() {
    ...
    const explicitlyConfigured = raw.enabled === true && configuredQueueRoot.length > 0 && configuredWorkspaceRoot.length > 0;
    const dssRoot = raw.enabled === false ? "" : detectDssRoot();
    const enabled = explicitlyConfigured || dssRoot.length > 0;
    const queueRoot = explicitlyConfigured ? path.resolve(configuredQueueRoot) : dssRoot ? path.join(dssRoot, ".agent-tasks") : "";
    const workspaceRoot = explicitlyConfigured ? path.resolve(configuredWorkspaceRoot) : dssRoot;
